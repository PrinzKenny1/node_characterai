...
class Requester {
        ...
        try {
          ...
          async function check() {
            if (pass) {
              ...
              if (waiting) {
                console.log(
                  `[node_characterai] Puppeteer - Currently in cloudflare's waiting room. Time left: ${waitingRoomTimeLeft}`
                );
              } else {
                resolve();
          ...
          console.log(
            "[node_characterai] Puppeteer - There was a fatal error while checking for cloudflare's waiting room"
          );
          console.log(error);
        }
      });
  ...
  async uploadImage(options, buffer) {
    ...
    try {
      ...
      if (!this.#hasDisplayed) {
        console.log(
          "[node_characterai] Puppeteer - Image-uploading is an experimental feature and may have bugs. Please report any issues on github"
        );
        this.#hasDisplayed = true;
      }

      response = await page.evaluate(
        async (payload, url, mime) => {
